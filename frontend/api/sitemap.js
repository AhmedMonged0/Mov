...
export default async function handler(req, res) {
  try {
    ...
    const [trendingRes, popularRes] = await Promise.all([
      fetch(${BASE_URL}/trending/movie/day?api_key=&language=ar),
      fetch(${BASE_URL}/movie/popular?api_key=&language=ar&page=1)
    ]).catch(() => [null, null]);
    ...
    const genreRoutes = genres.map(g => ({
      url: https://movora.me/categories?genre=,
      changefreq: 'weekly',
      priority: '0.7'
    }));
    ...
    uniqueMovies.forEach(movie => {
      ...
      xml +=   <url>\n;
      xml +=     <loc>https://movora.me/movie/</loc>\n;
      xml +=     <lastmod></lastmod>\n;
